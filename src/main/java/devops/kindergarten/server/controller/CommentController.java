...
public class CommentController {
	...
	public List<CommentResponseDto> findAllByPageIdOrPageName(@RequestParam(value = "id") Optional<Long> pageId,
		@RequestParam(value = "name") Optional<String> pageName) {
		if (pageId.isEmpty() && pageName.isPresent()) {
			if(TheoryPageName.isCorrectPageName(pageName.get())){
				return commentService.findAllByPageName(TheoryPageName.valueOf(pageName.get().toUpperCase()));
			}
